...
function loadDishMenuData(page, recordNumber, c)
{
    ...
    xmlhttp.open("GET", "../menuManagement/model/loadDishMenuData.php?page=" + page + "&recordNumber=" + recordNumber + "&c='%" + c + "%'&menuForDate='"+menuForDate +"'", true);
    xmlhttp.send();
}
...
function addNewDishIntoMenu(dishNumber, menuForDate)
{
    ...
    if (confirm("Bạn chắc chắn muốn thêm món ăn này vào Menu ??? "))
    {
        ...
        xmlhttp.open("GET", "../menuManagement/model/addNewDish.php?dishNumber=" + dishNumber+"&menuForDate='"+menuForDate+"'", true);
        xmlhttp.send();
        loadMenuData(1, menuForDate);
        loadDishMenuData(1,10,'');
...
//    {
//        xmlhttp.open("GET", "../menuManagement/model/deleteADish.php?dishNumber=" + dishNumber, true);
...
function loadMenuData(page, menuForDate)
{
    ...
    xmlhttp.open("GET", "../menuManagement/model/loadMenuData.php?page="+page+"&menuForDate='"+menuForDate+"'", true);
    xmlhttp.send();
    loadDishMenuData(1, 10, '');
}
...
function deleteADishFromMenu(ID, menuForDate)
{
    ...
    if (confirm("Bạn chắc chắn xóa món ăn này khỏi menu ??? "))
    {
        ...
        xmlhttp.open("GET", "../menuManagement/model/deleteADish.php?ID=" + ID, true);
        xmlhttp.send();
        loadMenuData(1, menuForDate);
        loadDishMenuData(1,10,'');
