...
$i = 0;
$dishCount = 0;
$mysql = "SELECT * FROM dishOrderDetail WHERE userName = $userName AND menuForDate = $menuForDate";
$mysqlDishCount = "SELECT count(dishNumber) as count FROM dishOrderDetail WHERE userName = $userName AND menuForDate = $menuForDate";
...
if ($link) {
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $typeButton = 'success';
        $buttonName = 'Đặt món';
        $event = 'onclick="openSecondPage(' . $row["dishNumber"] . ', ' . $menuForDate . ')"';
